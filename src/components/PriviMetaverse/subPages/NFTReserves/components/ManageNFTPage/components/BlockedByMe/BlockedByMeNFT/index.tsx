...
export default ({ item }) => {
  ...
  const getRemainingTime = (_blockingInfo) => {
    let value = Math.max(_blockingInfo?.ReservePeriod * 3600 * 24 * 1000 + _blockingInfo?.created - Date.now(), 0)
    value = value / 1000;
  ...
  return (
    ...
    >
      ...
      >
        ...
        <Box display="flex" alignItems="center" flex={1}>
          <Box display="flex" flexDirection="column" flex={0.25} className={classes.section}>
            ...
            <Box>
              {`${item.history?.Price} ${getTokenSymbol(item.history?.PaymentToken)}`}
            </Box>
          </Box>
          <Box display="flex" flexDirection="column" flex={0.25} pl={6} className={classes.section}>
            ...
            <Box>
              {`${item.history?.Price * (item.history?.TotalCollateralPercent || item.history?.CollateralPercent) / 100} ${getTokenSymbol(item.history?.PaymentToken)}`}
            </Box>
          </Box>
      ...
      <img
        ...
        onClick={() => {history.push(`/reserve/explore/${item.token_address}/${item.token_id}`)}}
      />
    </Box>
  )
}