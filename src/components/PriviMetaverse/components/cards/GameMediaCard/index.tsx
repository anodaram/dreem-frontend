...
import GameMediaDetailModal from "components/PriviMetaverse/modals/GameMediaDetailModal";
...
export default function GameMediaCard(props) {
  ...
  const { media_id: mediaId } = useParams<{ media_id: string }>();

  const [media, setMedia] = React.useState<any>(props.item ?? {});
  const [openMediaDetailModal, setOpenMediaDetailModal] = useState<boolean>(false);
  ...
  React.useEffect(() => {
    if (item?.id) {
      ...
      if (mediaId) {
        ...
        setOpenMediaDetailModal(item.id == decodeURIComponent(mediaId));
      }
    }
  }, [item?.id, mediaId, gameInfo]);

  const handleCloseMediaDetailModal = () => {
    setOpenMediaDetailModal(false);
  };
  ...
  return (
    <Box className={classes.card}>
      ...
      {openMediaDetailModal && (
        <GameMediaDetailModal
          open={openMediaDetailModal}
          nft={media}
          onClose={handleCloseMediaDetailModal}
          onFruit={updateFruit}
        />
      )}
    </Box>
