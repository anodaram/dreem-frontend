...
export default function AvatarCard(props) {
  ...
  const [openCharacterDetailModal, setOpenCharacterDetailModal] = useState<boolean>( false);
  ...
  React.useEffect(() => {
    setOpenCharacterDetailModal(itemId && itemId == media?.versionHashId ? true : false)
  }, []);

  const handleCloseCharacterDetailModal = () => {
    setOpenCharacterDetailModal(false);
  };
    ...
    <Box className={classes.card}>
        ...
        <>
          ...
          <Box p={1.5}>
            <PrimaryButton
              ...
              onClick={() => {
                setOpenCharacterDetailModal(true);
              }}
            >
              see more
            </PrimaryButton>
      ...
      {openCharacterDetailModal && (
        <CharacterDetailModal
          ...
          id={media.id}
          realmData={item?.realm}
          onClose={handleCloseCharacterDetailModal}
          onFruit={updateFruit}
        />
      )}
    </Box>
