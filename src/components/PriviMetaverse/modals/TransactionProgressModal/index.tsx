...
export default function TransactionProgressModal({
  ...
  return (
    <Modal showCloseIcon isOpen={open} onClose={onClose} className={classes.root} size="small">
      {txSuccess === true ? (
        <img src={require("assets/metaverseImages/result_success.png")} />
      ) : txSuccess === false ? (
        <img src={require("assets/metaverseImages/result_fail.png")} />
      ) : (
        <div style={{ position: "relative" }}>
          <img className={classes.loader} src={require("assets/metaverseImages/loading.png")} />
          <div className={classes.ethImg}>
            <img
              src={require(`assets/chainImages/${isEth ? "eth" : "Polygon"}.png`)}
              width="100%"
              height="100%"
            />
          </div>
      ...
      {hash && (
        <>
          ...
          <PrimaryButton
            size="medium"
            style={{ background: Color.Purple, marginTop: "24px" }}
            isRounded
